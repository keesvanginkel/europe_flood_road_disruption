...
def optimal_routes():
    ...
    network_files = [os.path.join(networks_europe_path, f) for f in os.listdir(networks_europe_path) if
                     f.endswith('-edges.feather')]
    ...
    country_codes = [f.split('-edges')[0] for f in os.listdir(networks_europe_path) if f.endswith('-edges.feather')]
    ...
    for i in [7]:  #range(len(network_files)):
        ...
        if selected_centroids.empty:
            print(current_country, 'is not an EU memberstate.')
            continue
        ...
        # create the graph
        G = graph_load(network, ['geometry', 'id'])  # ['geometry', 'RP100_cells_intersect', 'RP100_max_flood_depth']
        G.summary()
        ...
        pref_routes_df.to_feather(os.path.join(output_folder.format(current_country), 'optimal_routes_{}_{}.feather'.format(weighing, current_country)))
        ...
        # gdf_to_shp(gpd.GeoDataFrame(network, geometry='geometry', crs='epsg:4326'),
        #            os.path.join(output_folder.format(current_country), 'edges_{}.shp'.format(current_country)))


if __name__ == '__main__':
    import timeit
    print(timeit.timeit("optimal_routes()", setup="from __main__ import optimal_routes", number=1))
    # optimal_routes()
